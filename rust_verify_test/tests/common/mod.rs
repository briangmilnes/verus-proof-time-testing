...
pub fn run_verus(
    ...
    // Use VERUS_TARGET_PATH env var or default to known location
    let verus_target_path = std::env::var("VERUS_TARGET_PATH")
        .map(std::path::PathBuf::from)
        .unwrap_or_else(|_| std::path::PathBuf::from("/home/milnes/projects/verus-lang/source/target-verus/release"));
    ...
    // Use standalone verus binary (handles library paths internally)
    ...
    // Import our library if it's been compiled
    let lib_dir = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).parent().unwrap().join("target").join("verus");
    let lib_rlib = lib_dir.join("libverus_proof_time_testing.rlib");
    let lib_vir = lib_dir.join("verus_proof_time_testing.vir");
    if lib_rlib.exists() && lib_vir.exists() {
        verus_args.append(&mut vec![
            "--extern".to_string(),
            format!("verus_proof_time_testing={}", lib_rlib.to_str().unwrap()),
            "--import".to_string(),
            format!("verus_proof_time_testing={}", lib_vir.to_str().unwrap()),
        ]);
    }
